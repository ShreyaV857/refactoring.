...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        final NumberFormat form = NumberFormat.getCurrencyInstance(Locale.US);
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += Constants.COMEDY_OVER_BASE_CAPACITY_AMOUNT * (p.getAudience() - Constants.TRAGEDY_AUDIENCE_THRESHOLD);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(), form.format(thisAmount / Constants.PERCENT_FACTOR), p.getAudience()));
            totalAmount += thisAmount;
        }
        result.append(String.format("Amount owed is %s%n", form.format(totalAmount / Constants.PERCENT_FACTOR)));
